...
async def current_version(session: SessionDep) -> dict:
    version, _p, _t = await legal_service.current(session)
    return {"version": version}


@router.get("/privacy", response_model=LegalOut)
async def privacy(session: SessionDep) -> LegalOut:
    version, privacy_md, _t = await legal_service.current(session)
    return LegalOut(
...
async def terms(session: SessionDep) -> LegalOut:
    version, _p, terms_md = await legal_service.current(session)
    return LegalOut(
