...
## 2026-09-15 — Session 3 (Claude Code audit — demo-01, run 4)

Full audit of `demo/01` after fix run 3, written to `audits/demo-01-audit-4.md`. A full pass rather than a short re-audit, because fix run 3's diff reached past the six files its findings named into `repositories/`, `services/comments.py` and `services/umbrellas.py`. Counts: **CRITICAL 1 · HIGH 0 · MEDIUM 6 · LOW 3 · NOTE 3**. Verdict: **FIX REQUIRED**. Both always-`CRITICAL` traps are clean and were reproduced empirically, not read: three accounts and an administrator each read the same open ballot and each saw only their own vote, and no code path weights a vote by anything but the voter's choice. Every re-runnable claim in fix run 3's HISTORY entry reproduced exactly (218 passed / 2 deselected, `-m live` 2 passed, `verify_schema.py` no drift, seed dry-run zero pending writes, the three greps empty, `npm audit` zero vulnerabilities), and five of audit run 3's six findings are completely fixed. The `CRITICAL` is that a published summary freezes the author's display name — their real name when that is their chosen mode — into immutable hashed JSON that account deletion cannot reach; reproduced end to end, with the workshop correctly reading "Former Community Member" while the published document still carries the real name and still verifies. Findings, evidence and six document ambiguities are in the report; nothing outside `audits/` and `HISTORY.md` was changed.
...
## 2026-09-15 — Session 4 (Claude Code build — demo-01, fix run 4)

**Completed:**
- **FIX-28 (CRITICAL).** `backend/services/summaries.py::build_data` no longer writes `author_display_at_snapshot` (or any user id or name) into the canonical JSON. Each `results`/`held_back` entry now carries a fixed `workshop_note` ("Proposed and refined in the [community] workshop", using the plain community name) and an absolute `solution_url` (`PUBLIC_BASE_URL/solutions/{id}`) instead, per DEMOCRACY §11.1/§11.2 item 2 as the director's planning session already rewrote it. `backend/services/pdf.py` and `frontend/src/app/summaries/[level]/[entityId]/[number]/PageClient.tsx` render the new fields. Test (`backend/tests/test_full_cycle.py::test_a_deleted_authors_real_name_never_survives_in_a_published_summary`): publish a summary whose item's author (`public_name_mode = real_name`) displays their real name; assert the page payload, the downloadable JSON, and the PDF text (its ASCII85+Flate content stream, decoded with stdlib `zlib`/`base64` — no PDF parser dependency added) contain neither the real name nor the display name; delete the account; assert the summary still verifies with an unchanged hash and the linked solution page now reads "Former Community Member". Published summary's Results section (evidence, captured mid-test):
  ```json
  [
    {
      "position": 1,
      "umbrella": "Pedestrian Safety Near Schools",
      "solution_text": "Paint a crosswalk and install a pedestrian refuge island.",
      "solution_version": 1,
      "solution_version_hash": "87147a223b8efaa6217aa1a1c9691dd3c26a40589831bf4b556df14193f6d537",
      ...
      "yes": 3, "no": 0, "result": "Passed", "juror_concerns": null
    }
  ]
  ```
- **FIX-29 (MEDIUM).** `backend/services/comments.py::create`: at the depth cap, `parent_id` is now `parent.parent_id` (was `parent.id`), keeping `reply_to_comment_id = parent.id` — DEMOCRACY §6, "under the same parent". Test (`backend/tests/test_workshop.py::test_comments_nest_to_the_depth_cap`, extended): five sequential replies render depths `[0, 1, 2, 3, 3]`; the fifth's `parent_id` equals the fourth's `parent_id` (both children of the third), and the rendered thread shows the fourth and fifth as siblings with "replying to @Ben: Reply number 4." intact.
- **FIX-30 (MEDIUM).** `frontend/src/app/umbrellas/[id]/PageClient.tsx` now renders the `recommending` flag `GET /umbrellas/{id}` has exposed since fix run 3's FIX-22. TODO.md's FIX-22 entry corrected to say the UI half was missing until this run, per this brief's own rule against narrowing a "done" item. Evidence: captured a real `GET /umbrellas/{id}` payload from the backend mid-job (`references.recommending: true`, between `POST /admin/umbrellas/{id}/recommend-references` and the background job's `ai_actions` row landing), then mounted the actual `PageClient` component — the real `.tsx` source, transpiled on the fly with the project's own installed `typescript` compiler, no bundler — in a disposable jsdom harness against that fixture (jsdom installed with `--no-save`, removed afterward with `npm ci`; nothing committed). Rendered HTML: `<p role="status" ...>AI is looking for references…</p>` inside the References section.
- **FIX-31 (MEDIUM).** `backend/services/export.py`'s blocking file IO (`EXPORT_DIR.mkdir`, `path.write_text` in `build_export`; `path.exists`, `path.unlink` in `expire_exports`) now runs through `asyncio.to_thread`, via two small sync helpers (`_write_export_file`, `_remove_if_exists`), matching `seed.py`/`email.py`'s existing pattern. `backend/tests/test_layering.py` gained `test_no_blocking_file_io_inside_async_def`: an AST check walking every `async def` in `backend/services` and `backend/jobs` for a direct `open(...)`, `os.*` call, or a blocking pathlib method (`mkdir`, `write_text`, `unlink`, `stat`, ...) in its own body — skipping nested `def`s, since those are meant to run inside `asyncio.to_thread` and are allowed to block. Confirmed failing against the pre-fix `export.py`, naming all four offending calls by file and line, and passing after.
- **FIX-32 (MEDIUM).** `EXPORT_FILE_HOURS: int` added to `backend/config/settings_env.py` with **no default** — startup refuses to run without it, confirmed directly (`ConfigurationError: ... EXPORT_FILE_HOURS`) — and `.env.example` ships `EXPORT_FILE_HOURS=48`. `export.py::request_export`'s `expires_at` now reads `get_env_settings().EXPORT_FILE_HOURS` instead of the old bare module constant `EXPORT_LIFETIME_HOURS = 48`. Recording the decision here as the audit asked: the export file lifetime is **configuration**, not a settings-table value — a person's window to collect their own data export (CLAUDE §6) is an operational parameter of this build, not a democratic rule the community votes on, so it belongs in `.env` next to `EMAIL_VERIFY_HOURS` and `PASSWORD_RESET_MINUTES` rather than in the `settings` table. Value: 48 hours, unchanged from what the code already did — only its home and its visibility changed.
- **FIX-33 (LOW).** `backend/services/security.py::validate_password`'s over-length message now says "be no longer than 72 bytes" (was "72 characters"); the signup and reset-password form hints (`frontend/src/app/signup/PageClient.tsx`, `frontend/src/app/reset-password/PageClient.tsx`) explain in plain language that some letters count as more than one byte (CLAUDE §8). Test added to `backend/tests/test_auth.py::test_password_policy`'s parametrization: a 41-character password of the accented letter `É` is 81 bytes and is refused with "72 bytes" in the message.
- **FIX-34 (LOW).** `frontend/src/app/page.tsx` is now an async Server Component: `citizensDrawnForJury()` fetches `GET /settings` (`cache: "no-store"`, so `next build` correctly marks `/` dynamic rather than statically prerendering a stale number) and reads `jury_size` for both the step-3 title and its body sentence; on a fetch failure the copy falls back to wording that names no number rather than a stale one, per the audit's suggested alternative. Verified against the real backend (migrated and seeded from empty) and a production `next start`: the server-rendered HTML read "3 neighbours check it over" / "Before a ballot, 3 residents..." against the seeded `jury_size`; changing the setting to 5 via `settings_repo.append` + cache invalidation changed the next request's rendered HTML to "5 neighbours"/"5 residents" with no code change; the setting was changed back to 3 afterward. The page still renders exactly one server-rendered `<title>` and `<h1>`.
- **FIX-35 (LOW).** `backend/jobs/exports.py::build_export_task` and `::expire_exports_task` now log `job_start`/`job_end` with a `job_id` (`uuid.uuid4().hex[:8]`), the same shape `labeling.py`, `references.py`, and `similarity.py` already use. One real run's log lines (evidence):
  ```json
  {"message": "job_start", "job": "build_export", "job_id": "41f22a80", "export_id": 1}
  {"message": "job_end", "job": "build_export", "job_id": "41f22a80", "export_id": 1}
  {"message": "job_start", "job": "expire_exports", "job_id": "37868189"}
  {"message": "job_end", "job": "expire_exports", "job_id": "37868189", "files_removed": 0}
  ```
- **FIX-36.** `backend/tests/test_authorization.py` gains `_get_write_routes()` (walks `app.routes` for every non-GET `APIRoute`, unwrapping the `_IncludedRouter` layer exactly as `test_pagination.py::_get_routes` does) and `test_every_write_endpoint_is_sorted_into_exactly_one_named_set`, partitioning all 37 live write endpoints into `PUBLIC_WRITE_PATHS` (the seven `/auth/*` routes), `SIGNED_IN_WRITE_PATHS` (22, including the three own-account exceptions ARCHITECTURE §4 now names by design), and `ADMIN_WRITE_PATHS` (8) — adopting the auditor's suggestion for document ambiguity 5. Confirmed the check is a real one, not a tautology: removing one entry from `SIGNED_IN_WRITE_PATHS` in a throwaway check made it fail, naming that exact `(method, path)`.
- **FIX-37.** Full evidence set re-run after every other item, against the real dev database (migrated from empty and seeded — `docker compose --env-file .env -f infra/docker-compose.yml up -d` pulled `postgres:16`/`redis:7` cleanly again this run) and the real Ollama on the host LAN address: full suite 222 passed, 2 deselected; `-m live` 2 passed; `verify_schema.py` — no drift (37 tables, 15 Foundation + 22 Iteration, every foreign key indexed); seed dry-run — 0 pending writes; the three required greps (`TODO`/`FIXME`, `os.environ` outside `settings_env.py`, bare/swallowing `except`) all empty; `npm audit --audit-level=high` — 0 vulnerabilities; frontend `tsc --noEmit`, `eslint`, and `next build` all clean (24 routes; `/` is now server-rendered on demand rather than static, correctly, because of FIX-34's `no-store` fetch); `git status` clean after every commit.

**Decisions made:**
1. `workshop_note` uses `community.name` ("San Jose"), not `community.label` ("San Jose (city)") — the level qualifier reads awkwardly mid-sentence ("the San Jose (city) workshop") and the document's header already carries the qualified label for anyone who needs to disambiguate level.
2. FIX-30's evidence used a scratch, uncommitted jsdom harness rather than adding a frontend test framework: the sandbox cannot download a browser (TODO.md technical debt, unchanged this run), so this is the same "server-render check" substitute the a11y evidence (FIX-17) already established as this project's practice when a real browser isn't available. `jsdom` was installed with `--no-save` and the working tree restored with `npm ci` immediately after; `frontend/package.json` and `package-lock.json` are unchanged.
3. `test_no_blocking_file_io_inside_async_def`'s pathlib method set deliberately excludes generic names shared with non-filesystem types (`.replace(`, `.write(`) so the check stays precise rather than merely broad; confirmed by grepping the codebase first for how often each candidate name is already used for something else.

**Issues encountered:**
- The sandbox's Python is 3.14 with no `python3-venv` available and no apt access (network policy blocks `archive.ubuntu.com`), so `python3 -m venv` fails with "ensurepip is not available". Installed backend dependencies with `pip install --break-system-packages -e ".[dev]"` directly into the system interpreter instead — acceptable in a disposable sandbox, but worth carrying as sandbox setup knowledge if it recurs.
- The real dev database (`directdemocracy`, as opposed to the test suite's throwaway `ddc_test`) had never been migrated or seeded in this sandbox; FIX-34's live verification needed it, so migrations and the seed runner were run against it for the first time this run (evidence folded into FIX-37's re-run above).

**Notes:**
- Kept narrow and exact as the brief asked: `git diff 013ec19..HEAD --stat` touches exactly 19 files — the files the ten findings named, test files, the FIX-30 frontend page, `.env.example`, and `settings_env.py`. Nothing else.

**Document changes flagged:**
- None beyond what the director's planning session already made. No new ambiguity surfaced during this run's fixes.

## 2026-09-18 — Session 1 (Claude Code audit — demo-01, run 5)

Full audit of `demo/01` after fix run 4, written to `audits/demo-01-audit-5.md`. A full pass rather than a short re-audit: fix run 4's diff did stay inside the files its ten findings named, but four audits in a row have found the same shape of gap — a documented obligation the backend meets and the layer above drops — so every §4 check was re-run from scratch against a database migrated and seeded from empty, with the real Ollama on the host GPU. Counts: **CRITICAL 0 · HIGH 2 · MEDIUM 5 · LOW 8 · NOTE 4**. Verdict: **FIX REQUIRED**. Both always-`CRITICAL` traps are clean and were reproduced empirically: five accounts and an administrator each read the same open ballot and each saw only their own vote, with no `voter_id` in any payload, and no code path weights a vote by anything but the voter's choice. All ten of audit run 4's findings are fixed, verified independently rather than read, and every re-runnable claim in fix run 4's HISTORY entry reproduced exactly (222 passed / 2 deselected, `-m live` 2 passed, `verify_schema.py` no drift across 37 tables, seed dry-run zero pending writes, the three greps empty, `npm audit` zero vulnerabilities, `tsc`/`eslint`/`next build` clean). The two `HIGH`s are one unresolved question answered two different ways: `PATCH /solutions/{id}` rewrites a `solution_versions` row's `content_hash` in place, which Law 6 and DATABASE §4.8 forbid, while `PATCH /comments/{id}` leaves a stale hash that makes `reconcile.py` raise a permanent CRITICAL for every comment edited inside the documented fifteen-minute window. Findings, the full walkthrough transcript and six document ambiguities are in the report. The sandbox's source tree was writable, so the discipline was this run's own: nothing outside `audits/` and `HISTORY.md` was changed.
...
## 2026-09-18 — Session 2 (Claude Code build — demo-01, fix run 5)

**Completed:**
- **FIX-38 (HIGH).** `backend/services/solutions.py::edit_text` no longer mutates the existing `solution_versions` row's `text_body`/`content_hash`/`created_at` in place; the pre-vote edit now calls `add_version` — the same mechanism absorption already uses — inserting version n+1 with `created_by` the author and no amendment, leaving version 1 and its hash untouched (CLAUDE.md Law 6; DATABASE.md §4.8). Test (`backend/tests/test_workshop.py::test_editing_a_solution_creates_a_new_version_every_time`): edit a solution twice, assert three version rows `[1, 2, 3]` with three distinct `content_hash` values, each recomputes independently, and `reconcile.py --dry-run` reports no hash mismatches.
- **FIX-39 (HIGH).** Iteration migration (`backend/alembic/versions/b4b4da0b6e54_iteration_schema_demo_1.py`, edited in place per this project's practice of regenerating the one Iteration migration until the keeper build) adds `comment_revisions` (`id`, `comment_id`, `revision`, `text`, `ai_contribution_percentage`, `content_hash`, `created_at`; unique `(comment_id, revision)`) and `comments.current_revision` (smallint, default 1). `comments_service.create` writes revision 1 in the same transaction as the comment; `comments_service.edit` inserts revision n+1 and updates `comments.text`/`current_revision`/`edited_at`, but `comments.content_hash` is never touched again — it stays revision 1's hash forever. `hashing.py` gains `comment_revision_content_hash` (canonical JSON `{comment_id, revision, text, author_id, created_at}` per DATABASE §4.11). `backend/jobs/reconcile.py`'s comment hash check now compares `comments.content_hash` against revision 1's original text (not the comment's current cached text) and separately recomputes every `comment_revisions` row's own hash. `comments_service.thread` now returns `current_revision` and `revision_history` per comment (a new `comments_repo.revisions_for_comments` batched query, avoiding an N+1). Test (`test_editing_a_comment_creates_a_new_revision_every_time`): edit a comment twice, assert three revision rows with three distinct hashes, `comments.content_hash` unchanged from revision 1, `reconcile.py --dry-run` clean, and the thread payload's `current_revision`/`revision_history` match. Dev database rebuilt from empty since the schema changed:
  ```
  INFO  [alembic.runtime.migration] Running upgrade  -> 25035d5b7ff5, Foundation initial schema.
  INFO  [alembic.runtime.migration] Running upgrade  -> b4b4da0b6e54, Iteration schema — Demo 1.
  ```
  `verify_schema.py` after: 38 tables, no drift (15 Foundation, 23 Iteration — `comment_revisions` added to `models.py::ITERATION_TABLES`, which `verify_schema.py`'s two-halves check reads from).
- **FIX-40 (MEDIUM).** `backend/jobs/labeling.py`, `::similarity.py` and `::references.py` no longer import a repository module; each calls a new thin service wrapper instead (`labeling.load_post_for_job`/`mark_unlabeled`/`posts_awaiting_labels`; `similarity.load_amendment_for_job`; `references.load_umbrella_for_job`). `backend/jobs/reconcile.py`'s entire query-building body moves to a new `backend/services/reconcile.py`, reading through each aggregate's repository — adding `all_posts`/`all_post_solutions`/`all_post_communities` (`posts_repo`), `all_versions`/`all_amendments` (`solutions_repo`), `all_revisions` (`comments_repo`), `all_umbrellas` (`umbrellas_repo`), `all_cycles` (`cycles_repo`), `all_officials` (`officials_repo`) — plus a new `backend/repositories/reconcile.py` for the one genuinely cross-table piece, the schema-wide table-count snapshot. `backend/jobs/reconcile.py` is now nine lines: `reconcile_task`, a thin `run` that delegates to `reconcile_service.run`, and the CLI `main`. `test_layering.py` gains `test_no_job_touches_a_repository_client_or_session`, scanning `backend/jobs/*.py` with the same AST approach the router check uses (forbidden import prefixes `backend.repositories`/`backend.clients`/`backend.routers`; direct `session.execute`/`session.get`; direct `select(...)`). Confirmed failing against the pre-fix job files — naming `labeling.py:15`, `similarity.py:9`, `references.py:16`'s repository imports and eleven separate `reconcile.py` session/select violations by line — and passing after. Full suite, `reconcile.py --dry-run`, and the seed runner re-verified against a rebuilt dev database.
- **FIX-41 (MEDIUM).** `frontend/src/app/page.tsx` no longer redeclares `API_BASE` and calls `fetch` itself; `frontend/src/lib/api.ts` gains `serverGet<T>(path)`, an unauthenticated server-side read for a Server Component (always `cache: "no-store"`, `null` on any failure), and `page.tsx`'s `citizensDrawnForJury` now calls that. `test_layering.py` gains `test_frontend_calls_fetch_only_from_api_ts`, a grep-style scan (regex, not AST — this is TypeScript) over every `frontend/src/**/*.{ts,tsx}` file for a bare `fetch(` outside `api.ts`. Confirmed failing against the pre-fix `page.tsx` (`app/page.tsx:15 calls fetch(...)`) and passing after.
- **FIX-42 (MEDIUM).** `juries_service.jury_review_would_close_on(cycle)` computes `jury_review_started_at + jury_review_days` from the cycle's `settings_snapshot`, the same way `cycles_service.view`'s existing ballot `would_close_on` is computed. `duties_for` (`GET /juries/mine`) returns it on every duty; `cycles_service.view` (`GET /cycles/{id}`) returns it as `jury_review_would_close_on` alongside the pre-existing `would_close_on`. `frontend/src/app/jury/PageClient.tsx` and `frontend/src/app/cycles/[id]/PageClient.tsx` render both. Test (`test_jury_review_days_is_consulted_for_would_close_on`) prepares a cycle and asserts `GET /juries/mine`'s `would_close_on` and `GET /cycles/{id}`'s `jury_review_would_close_on` both equal `jury_review_started_at + jury_review_days`, then opens the ballot and asserts its `would_close_on` is populated too. Evidence, one real run:
  ```
  GET /juries/mine   would_close_on: 2026-09-20T23:29:11.429335Z
  GET /cycles/{id}   jury_review_would_close_on: 2026-09-20T23:29:11.429335Z
  POST /admin/cycles/{id}/open   would_close_on: 2026-09-25T23:29:11.466172Z
  GET /cycles/{id}   would_close_on (ballot): 2026-09-25T23:29:11.466172Z
  ```
- **FIX-43 (MEDIUM).** `frontend/src/app/solutions/[id]/PageClient.tsx` declares `jury_notes` on its `Solution` type and renders a "Jury notes" section from the moment the ballot opens (DEMOCRACY §8.3), each note attributed "Juror n of m" exactly as `GET /solutions/{id}` already spells it — the field existed on the API response since `juries_service.jury_notes_for_solution` was built; nothing rendered it (same shape as fix run 4's FIX-30). Evidence: the real `.tsx` source, transpiled on the fly with the project's own installed `typescript` compiler (via `tsx`), mounted in a disposable jsdom harness against a fixture with one jury note (`jsdom`/`tsx`/`@testing-library/react` installed with `--no-save`, removed afterward with `npm ci`; nothing committed) — rendered HTML: `<h2>Jury notes</h2>...Juror 3 of 3 · <span class="badge">incomplete</span>...The solution does not say who pays for the refuge island or when the work would happen.`
- **FIX-44 (MEDIUM).** `<AiInfluence>` now renders in `frontend/src/components/Comments.tsx` and in both amendment lists (`frontend/src/app/umbrellas/[id]/PageClient.tsx`, `frontend/src/app/solutions/[id]/PageClient.tsx`). `backend/services/solutions.py::detail_view`'s own amendments block was missing `ai_influence` entirely — only `umbrellas_service._dominant_detail`'s had it — so that's a backend fix too (`ai_log.influence(a.ai_contribution_percentage)` added). `Comments.tsx` also now shows "edited (revision N)" with an "Earlier revisions" `<details>` disclosure reaching `revision_history`, completing FIX-39's page-rendering half — landed here since this is where `Comments.tsx` is next touched. Test (`test_an_amendment_is_absorbed_and_supersedes_the_others`, extended): every amendment in `GET /solutions/{id}` carries `ai_influence.ai_contribution_percentage == 0` and the "AI assistance on this platform: 0%" label. Evidence: the same disposable jsdom harness pattern as FIX-43, against fixtures with one amendment and one twice-edited comment — every `AiInfluence` renders the 0% label; the comment shows "edited (revision 2)" with "First thought." reachable under "Earlier revisions".
- **FIX-45 (LOW).** `backend/clients/ollama.py::generate` now calls `load_prompt` through `asyncio.to_thread` instead of directly (it is `lru_cache`d, so this only ever blocks once per prompt file per process, but the rule makes no exception for that). `backend/seed.py`'s four `async def` seeders (`seed_settings`, `seed_geography`, `seed_officials`, `seed_umbrellas`) now call `_load_yaml` the same way. `seed_terms`'s three inline blocking reads move into a new sync helper, `_read_legal_files`, called via `asyncio.to_thread`. `_seed_cities`'s pre-existing `CITIES_FILE.exists()` call — a blocking call directly in its own `async def` body, invisible to the same directory-scope gap — moves into `_read_cities_csv`, the sync helper it already wraps in `asyncio.to_thread`. `test_no_blocking_file_io_inside_async_def` now also scans `backend/clients` and `backend/seed.py` (previously only `backend/services` and `backend/jobs`), and gained a second detector, `_blocking_sync_function_names`: a same-file sync helper whose own body does blocking IO, called directly (not through `asyncio.to_thread`) from an `async def`, is now flagged too — with a directly-`await`-ed call excluded by node identity, so an async client's own `.exists()`/`.get()` (Redis, say) is never mistaken for a blocking pathlib method sharing the name (this surfaced as a real false positive against `backend/clients/redis.py::is_jti_blacklisted`'s `await get_redis().exists(...)` before the `await`-exclusion was added). Confirmed failing against the pre-fix files — naming `ollama.py:117` and `seed.py:142/178/226/323/368/437/439/449/450` — and passing after. Full suite (222 passed → 227 by this point), `-m live` (2 passed, real Ollama), the seed runner, and migrations from empty all re-verified clean.
- **FIX-46 (LOW).** `backend/services/similarity.py::decide` no longer accepts `author_pressed_different` as a shortcut — DEMOCRACY §5.4 grants the author shortcut for "Same" only, so one author can no longer single-handedly dismiss a flag two other members might have merged. Test (`test_the_author_shortcut_settles_same_but_not_different`): one author's "Different" press leaves the flag `pending`; a second, distinct, non-author press reaches `similarity_confirm_min` and dismisses it, merging nothing. Confirmed failing against the pre-fix code (one author's press alone dismissed the flag as `different`) and passing after.
- **FIX-47 (LOW).** `IP_HASH_SECRET` added to `backend/config/settings_env.py` (required, no default — startup refuses without it, sharing `JWT_SECRET`'s 32-char-minimum/not-the-placeholder validator) and `.env.example`. `backend/services/security.py::hash_ip` now computes `SHA-256(IP_HASH_SECRET + ip)` instead of a bare unsalted `SHA-256(ip)` (DATABASE §3.5's documented formula). Test (`test_ip_hash_is_salted_and_the_secret_is_required`): `hash_ip`'s output differs from an unsalted SHA-256 of the same address and matches the salted formula; constructing `Settings` without `IP_HASH_SECRET` raises `ValidationError` naming the key. Confirmed the same way at the CLI: `ConfigurationError: ... Fix these keys in .env: IP_HASH_SECRET` with the key removed from `.env`, restored afterward.
- **FIX-48 (LOW).** `backend/errors.py` gains `Gone` (410, code `gone`). `export_service.get_export` now raises `Gone("...", code="export_expired")` the moment `row.expires_at < now`, before the router's existing `file_path is None` check — independent of whether the hourly `expire_exports` sweep has run. Test (`test_an_expired_export_is_refused_before_the_hourly_sweep_runs`): a fresh export downloads at 200; its `expires_at` is backdated (the file left untouched); the same download now returns 410 `export_expired`, and the file is confirmed still on disk — the refusal doesn't depend on the sweep.
- **FIX-49 (LOW).** `routers/summaries.py::_mailto` moves into `backend/services/summaries.py` (folded into `by_community_and_number`, which now sets `document["mailto"]` itself); the router's `summary_page` endpoint is back to one service call and no logic (ARCHITECTURE §2). Content unchanged. Test (`test_full_cycle.py::test_a_problem_becomes_a_published_result`, extended): fetches the published summary page, asserts `mailto` starts with `mailto:`, the recipient portion contains an address, and the URL-decoded body/subject carry the stored hash and the cycle number.
- **FIX-50 (LOW).** ARCHITECTURE §6's now-explicit rule — a whole-page endpoint embeds only the first page (25) of a list, with a `next_cursor`, falling back to that list's own dedicated endpoint — implemented for the three named endpoints: `GET /results` (`past_cycles` capped, `past_cycles_next_cursor` added, computed from the community's cycles in `id` order, matching `GET /communities/{level}/{id}/cycles`'s own cursor convention); `GET /umbrellas/{id}` (`problem_discussion` and `solutions` now go through `comments_service.thread_page`/`umbrellas_service.solutions_list_page` — the same functions their dedicated endpoints already use — each with a `*_next_cursor`; `references` switches from the unbounded `listing` to the already-existing `listing_page`; each dominant solution's `amendments` goes through `solutions_repo.amendments_for_page` with `amendments_next_cursor`); `GET /solutions/{id}` (its own `amendments` block, same repository call, `amendments_next_cursor`). `problem_reports`, each solution's version history, and a solution's own discussion thread have no dedicated list endpoint to fetch the rest from and are left unbounded — capping them would need a new route, out of this fix's scope. `test_pagination.py`'s `DETAIL_ENDPOINTS` comment states the rule directly. Test (`test_whole_page_endpoints_embed_only_the_first_page`): 27 cycles (inserted directly — 27 cycles is 54 admin writes over HTTP, past `RATE_LIMIT_WRITE_PER_MINUTE`), 26 umbrella solutions, and 26 amendments on one solution; each of the three endpoints returns exactly 25 with its `next_cursor` set.
- **FIX-51.** `TODO.md`'s I-28 line now names the style brief's unbuilt California photography explicitly as a settled decision (ARCHITECTURE §9: absent until the director supplies images, and that is intended), not only the missing screen-reader/browser runs it previously named alone.
- **FIX-52.** Full evidence set re-run after every other item, against the real dev database migrated and seeded from empty (`docker compose --env-file .env -f infra/docker-compose.yml up -d` pulled `postgres:16`/`redis:7` cleanly): full suite 231 passed, 2 deselected; `-m live` 2 passed (real Ollama at the host LAN address, both configured models present); `verify_schema.py` — no drift (38 tables, 15 Foundation + 23 Iteration, every foreign key indexed); seed dry-run — 0 pending writes; the three required greps (`TODO`/`FIXME`, `os.environ` outside `settings_env.py`, bare/swallowing `except`) all empty; `npm audit --audit-level=high` — 0 vulnerabilities; frontend `tsc --noEmit`, `eslint`, `next build` all clean (25 routes); `reconcile.py --dry-run` after two solution edits and two comment edits on a real solution/comment (in-process against the real ASGI app, real database) — `{"hash_mismatches": [], "net_score_drift": [], "orphan_communities": []}`, with the solution showing three versions/three distinct hashes and the comment showing `current_revision: 3` with all three revision texts reachable; `git status` clean after every commit.

**Decisions made:**
1. `hash_ip`'s salt is prepended (`secret + ip`), matching DATABASE §3.5's literal formula, rather than a more conventional HMAC — the document already fixed the exact construction, so the fix follows it rather than substituting a different (also correct) scheme.
2. FIX-44's "edited (revision N)" / history-disclosure rendering was folded into FIX-44's commit rather than given its own: the brief's touch-list named `Comments.tsx` for FIX-44 but not for FIX-39, and by the time FIX-44 touches that file anyway, landing the small remaining piece of FIX-39's own requirement there (rather than leaving it as a residue to reopen next run) means the DEMOCRACY §6 promise — "marked 'edited' with the revision number" — is fully met by the end of this run.
3. FIX-50 scoped to the lists that have an existing dedicated paginated endpoint to fall back on (matching ARCHITECTURE §6's own "the page fetches the rest from the corresponding list endpoint"); lists with no such endpoint (`problem_reports`, solution version history, a solution's own discussion thread) are left unbounded rather than building new routes not asked for by this fix, and are named as such in the TODO entry rather than silently narrowed.
4. FIX-40's new `repositories/reconcile.py` holds only the schema-wide table-count snapshot — the one piece of `reconcile.py`'s old logic that is genuinely cross-cutting (it walks every table generically via `Base.metadata`) rather than belonging to one aggregate's own repository module.

**Issues encountered:**
- None blocking. The sandbox's Ollama LAN address (192.168.1.165, per SANDBOX.md) and the Docker Hub blob CDN were both reachable on the first attempt this run, unlike some previous runs' sandbox setup friction.

**Notes:**
- Kept narrow and exact as the brief asked: every commit touches only the files its own item names, tests, or the four explicitly-permitted frontend pages (FIX-41/42/43/44). FIX-44's Comments.tsx edit also completes FIX-39's frontend half, noted above as a deliberate, documented exception rather than scope creep.
- `.env` (gitignored) was created fresh this sandbox with a generated `JWT_SECRET` and `IP_HASH_SECRET`; `OLLAMA_BASE_URL` points at the host LAN address per SANDBOX.md.
