...
- Users can delete their account at any time. All personal identifying
  information — name, email, password, date of birth, gender, political
  party — is permanently erased. Home city and county are retained at
  that granularity so the civic record stays in the right community;
  they are not identifying on their own. Posts, solutions, comments,
  votes, and AI-correction data are anonymized and attributed to
  "Former Community Member." The civic record is preserved; the
  identity is removed.
...
Nine documents define this project. Each has a single job. They do not
overlap.
...
| **AUDIT.md** | The audit-run procedure and severity scale. | When the method changes. | Director required. |
| **SANDBOX.md** | Docker Sandboxes setup and per-trial isolation. | As setup facts are confirmed. | Routine. |

Plus **briefs/** — one build brief per trial and the audit brief — and
**archive/** — superseded documents kept for reference, not
